...
// We are going to construct a robust port that supports BOTH Radix (matching generate_chart_svg.mjs exactly) and Transits.
...
export function generateChartSvg(data: any, options: ChartGeneratorOptions = {}): string {
  ...
  const CX = SIZE / 2;
  const CY = SIZE / 2;
  const RADIUS = SIZE / 2 - 40; // Exactly as Astro-Nex

  // Transits have a wider outer ring to fit two sets of planets
  ...
  const ASP_COLOR: Record<string, string> = {
    red: '#ee0000', blue: '#0000f7', green: '#00cc00', orange: '#ff8000'
  };
  ...
  // Zodiac Sign Boundaries (only span from midR to outerR)
  const SIGN_OFFSET = OFFSET % 30;
  ...
  if (data.houses) {
    ...
    let currentYear = data.birthYear || new Date().getFullYear() - 30; 
  ...
  for(let i=0; i<12; i++) {
    ...
    let textRot = visualAngle + 90;
    svgParts.push(\`<text x="\${p.x}" y="\${p.y}" fill="\${ZODIAC_COLORS_LIST[i%4]}" font-size="\${glyphFontSize}" text-anchor="middle" dominant-baseline="central" font-family="Astro-Nex" font-weight="normal" transform="rotate(\${textRot} \${p.x} \${p.y})">\${ZODIAC_GLYPHS[i]}</text>\`);
  }
  ...
  if (data.houses) {
    ...
    for(let i=0; i<12; i++) {
      ...
      // FIX: CUSP LINES ONLY START AT outerR AND EXTEND OUTWARD! THEY MUST NOT CROSS THE CHART!
  ...
  const drawAspects = (aspects: any[], pList1: any[], pList2: any[], opacity: string) => {
    for(const asp of aspects) {
      const p1d = pList1.find((p:any) => p.index === asp.p1);
      const p2d = pList2.find((p:any) => p.index === asp.p2);
      if(!p1d || !p2d) continue;
      ...
      const p1_outer = polar(innerR, p1d.longitude);
      const p2_outer = polar(innerR, p2d.longitude);
      ...
      if (isUnilateral) {
        ...
        svgParts.push(\`<line x1="\${fX1}" y1="\${fY1}" x2="\${xx}" y2="\${yy}" stroke="\${color}" stroke-width="\${strokeW}" stroke-dasharray="4,3,12,4,18,5,24,6,30,6,36,6,48,6,60,6" opacity="\${opacity}"/>\`);
        svgParts.push(\`<line x1="\${xx}" y1="\${yy}" x2="\${fX2}" y2="\${fY2}" stroke="\${color}" stroke-width="\${strokeW}" opacity="\${opacity}"/>\`);
      } else {
        const xx = (x1 + x2) / 2;
        const yy = (y1 + y2) / 2;
        const angle = Math.atan2(y2 - y1, x2 - x1);
        
        const scl = aspR * 0.00065;
        ...
        svgParts.push(\`<path d="\${pathData}" fill="\${color}" stroke="\${color}" stroke-width="0.55" opacity="\${opacity}"/>\`);
      }
    }
  };
