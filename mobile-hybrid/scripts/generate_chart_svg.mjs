...
// --- Zodiac wheel
const outerR = RADIUS * 0.84; // R_RULED_MID (Outer boundary for ticks)
const midR   = RADIUS * 0.78; // R_RULED_OUTER (Separates signs from degree marks)
const innerR = RADIUS * 0.65; // R_RULED_INNER (Inner boundary for zodiac)
const planetRingOuter = RADIUS * 0.48; // R_INNER
const aspR   = RADIUS * 0.435; // R_ASP

let natalPlanetR = RADIUS * 0.48 + (RADIUS * 0.65 - RADIUS * 0.48)/2;
...
for(let h=0; h<12; h++) {
  ...
  const p1 = polar(outerR, boundaryDeg); // Sign boundaries go from outerR (0.84)
  const p2 = polar(innerR, boundaryDeg); // ONLY to innerR (0.65), they do NOT enter the planet ring!
...
let startYear = data.birthYear || 1888; 
...
for(let i=0; i<12; i++) {
  ...
  // Re-calculate the visual angle for text rotation
  const visualAngle = normDeg(180 - mid + OFFSET);
  
  // Tangent text: rotate by visualAngle + 90
  let textRot = visualAngle + 90;
  ...
  // Draw 10 and 20 degree marks
  [10, 20].forEach(degOffset => {
    const deg = startDeg + degOffset;
    const pDeg = polar(degreeLabelR, deg);
    const vAngle = normDeg(180 - deg + OFFSET);
    let tRot = vAngle + 90;
    svgParts.push(`<text x="${pDeg.x}" y="${pDeg.y}" fill="#666" font-size="${RADIUS * 0.035}" text-anchor="middle" dominant-baseline="central" font-family="sans-serif" transform="rotate(${tRot} ${pDeg.x} ${pDeg.y})">${degOffset}</text>`);
  });
}
...
for(let i=0; i<12; i++) {
  ...
  if (isCardinal) {
    ...
    textRot = 0;
    // Push the text further out so it doesn't overlap the line end
...
data.planets.forEach((p) => {
  ...
  // Always draw tick on inner ring for natal planets
