...
## 30 de julio de 2026 — calendario y diagrama en Análisis de pareja II

### Incidencia observada

En `Análisis de pareja II` el calendario y el diagrama de barras compartían un
fallo de posicionamiento: ya no aparecían abiertos al entrar en la carta, pero
al activarlos se mostraban en posiciones bajas o desplazadas hacia la derecha.
El comportamiento era reproducible en macOS, Windows y el entorno VNC, por lo
que no se trataba de un problema exclusivo de una plataforma.

Durante la revisión también se confirmó que:

- `Análisis de pareja I` ya presentaba correctamente los overlays;
- la lógica del Astro-Nex original no abría estos paneles por defecto;
- el problema estaba concentrado en la presentación extendida de `compo_two`.

### Corrección

Se descartaron dos aproximaciones intermedias:

- reanclar los overlays sólo con cambios de `hadjustment/vadjustment`, porque
  `compo_two` seguía recuperando desplazamientos internos después del relayout;
- sacar temporalmente los widgets sin aislar su integración completa, porque
  esa prueba llegó a dejar `Pareja I/II` en blanco.

La corrección final reestructura el montaje de los dos paneles:

- `ChangeDatePanel` y el contenedor del diagrama se desacoplan del
  `GtkLayout` scrolleable;
- ambos se añaden como overlays de un `Gtk.Overlay` que envuelve el
  `ScrolledWindow` principal;
- el lienzo de la carta continúa dentro del `ScrolledWindow` directo, con lo
  que no se reintroduce un `GtkViewport` intermedio;
- la sincronización de visibilidad sigue leyendo el estado real de la barra de
  herramientas para evitar overlays “pegados” al cambiar de carta.

Con esta estructura, `Pareja II` deja de arrastrar calendario y diagrama con
el scroll interno de la carta extendida.

### Verificación

Se mantuvieron y ajustaron las pruebas GTK3 relevantes:
...
Además, se reconstruyó el instalador:
...
El área desplazable del navegador usa una altura solicitada de 330 píxeles
