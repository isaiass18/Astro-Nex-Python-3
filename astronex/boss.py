...
class Manager(object):
    ...
    def __init__(self,app,opts,state):
        ...
        acpaths_file = path.joinpath(app.appath, 'astronex/resources/ac.pk')
        with open(acpaths_file, 'rb') as source:
            self.acpaths = pickle.load(source, encoding='latin1')
    ...
    def set_mainwin(self,mainwin):
        ...
        # ``init_pools`` runs before GTK enters its main loop on startup.
        # Queue a second draw once the canvas has its final allocation; without
        # this GTK 3 can leave the first chart as a blank white surface.
        from .gobject_compat import idle_add
        idle_add(self.da.redraw)
