...
class DrawMixin(CoreMixin,ProgMixin,ProfileMixin,BioMixin,DiagramMixin,SheetMixin,PaarWabeMixin,PlanetogramMixin):
    ...
    # Interactive planetogram dialogs can toggle this.  Non-interactive
    # renderers (PNG/PDF/batch) require the same default explicitly.
    useagecircle = False
    shadow = True
    personlines = False
    turnpoints = True
    ...
    def get_nw(self,filter=None):
        ...
        if op in ['draw_nat','draw_house','draw_nod','draw_soul','draw_local']:
            ...
            if  not filter:
                return []
            if getattr(getattr(boss, 'da', None), 'plselector', None):
                return self.notwanted 
            else:
