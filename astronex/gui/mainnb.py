    ...
    def on_menuitem_activate(self,menuitem):
        if menuitem.child.get_text() == _('Exportar carta'):
            ...
            if response == gtk.RESPONSE_OK:
                ...
                # FileChooser returns text paths on Python 3.
...
class ChartBrowser(gtk.VBox):
    def __init__(self,ap_path,font):
        ...
        # At 220 logical pixels the GTK3 row metrics show the same roughly
        # twelve people as the historical 1.2 panel.
    ...
    def on_chart_activated(self,view,path,col):
        model,iter = view.get_selection().get_selected()
        id = model.get_value(iter,1)
        # The cross-table search model has a third column with its source
        # table.  The normal table model has only the display name and row id.
        if model.get_n_columns() > 2:
            table = model.get_value(iter,2)
        else:
            table = self.tables.get_active_text()
