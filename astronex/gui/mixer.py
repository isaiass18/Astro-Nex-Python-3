...
class MixerPanel(gtk.HBox):
    ...
    def drag_data_get_data(self,treeview,context,selection,target_id,etime):
        ...
        # GtkSelectionData is byte-oriented in PyGObject.  The PyGTK
        # ``target`` attribute became a getter in GTK 3.
        target = selection.get_target()
        selection.set(target, 8, data.encode('utf-8'))
    ...
    def drag_data_received_data(self,treeview,context,x,y,selection,info,etime):
        ...
        for key in list(self.views.keys()):
            if key == treeview:
                ...
                othertab = self.views[key].get_active_text()
        self.changes = True
        if context.get_selected_action() == gtk.gdk.ACTION_MOVE:
            # Do the safety check before telling GTK that the source row was
            # deleted.  Otherwise a protected chart can disappear from the
            # source view even though the database still contains it.
    ...
    def make_admin_panel(self):
        ...
        hbox.pack_start(img)
        but = gtk.Button(_('_Crear tabla'), use_underline=True)
        ...
        img.set_from_file(imgfile)
        hbox.pack_start(img)
        but = gtk.Button(_('E_liminar tabla'), use_underline=True)
        ...
        img.set_from_file(imgfile)
        hbox.pack_start(img)
        but = gtk.Button(_('_Renombrar'), use_underline=True)
        ...
        hbox = gtk.HBox()
        but = gtk.Button(_('_Exportar  tabla'), use_underline=True)
        but.connect('clicked',self.on_table_export)
        hbox.pack_start(but)
        vbox.pack_start(hbox,False,False)
        
        hbox = gtk.HBox()
        but = gtk.Button(_('_Importar  tabla'), use_underline=True)
    ...
    def import_response(self,dialog,rid,entry,tentry,info):
        if rid == gtk.RESPONSE_NONE or rid == gtk.RESPONSE_DELETE_EVENT:
            ...
            try:
                ...
                imported = pickle.load(input, encoding='latin1')
            except IOError:
