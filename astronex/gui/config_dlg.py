...
class ConfigDlg(gtk.Dialog):
    ...
    def dlg_response(self,dialog,rid):
        if rid == gtk.RESPONSE_OK:
            conf = ConfigObj(self.config_file, encoding='utf-8', default_encoding='utf-8')
...
class ColorsPage(gtk.VBox):
    ...
    def color_set_cb(self,colbut,lbl):
        # Gtk.Color channels are 16-bit.  Astro-Nex configuration stores
        # standard six-digit RGB values; persisting the raw channels produced
        # twelve-digit colours after a GTK3 theme edit.
        color = colbut.get_color()
        r, g, b = (format(round(channel / 257), '02x')
