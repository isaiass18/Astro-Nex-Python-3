...
class EntryDlg(gtk.Dialog):
    ...
    def __init__(self, parent, calc=False):
        ...
        # Ctrl+E always starts a new person.  ``Current.person`` survives for
        # the life of the application, so reset both name fields explicitly.
        ...
        self.connect('delete-event', self.on_delete_event, parent)
    ...
    def close_dialog(self, dialog, parent):
        parent.entry = None
        dialog.destroy()
        return True

    def on_key_press_event(self, dialog, event, parent):
        if event.keyval == gtk.keysyms.Escape:
            return self.close_dialog(dialog, parent)
        return False
