...
DIAGNOSTICS_DIR = Path(os.environ.get("ASTRONEX_DIAGNOSTICS_DIR", Path(__file__).resolve().parent / "diagnostics"))
...
def _diagnostic_authorized() -> bool:
    """Keep desktop diagnostics separate from the mobile API credential."""
    expected = os.environ.get("ASTRONEX_DIAGNOSTIC_KEY")
    received = request.headers.get("X-AstroNex-Diagnostic-Key", "")
    return bool(expected) and hmac.compare_digest(received, expected)


def _diagnostic_browser_authorized() -> bool:
    expected = os.environ.get("ASTRONEX_DIAGNOSTIC_KEY")
    received = request.args.get("key", "")
    return bool(expected) and hmac.compare_digest(received, expected)
...
@app.post("/v1/diagnostics/startup-log")
def store_startup_log() -> Response:
    """Receive a bounded macOS startup report for remote troubleshooting."""
    if not _diagnostic_authorized():
    ...
    encoded_log = payload["log"].encode("utf-8", errors="replace")
    if not encoded_log or len(encoded_log) > MAX_DIAGNOSTIC_LOG_BYTES:
        return jsonify(error="invalid_log_size"), 400
    ...
    report_path = DIAGNOSTICS_DIR / f"macos-startup-{received_at}-{report_id}.json"
    report_path.write_text(
        json.dumps({"received_at": received_at, "client": safe_client, "log": encoded_log.decode("utf-8", errors="replace")}, ensure_ascii=False),
        encoding="utf-8",
    )
    return jsonify(status="stored", report_id=report_id), 201


@app.get("/diagnostics")
def diagnostics_index() -> Response:
    """Small browser view for reports while diagnostics uses an IP-only host."""
    if not _diagnostic_browser_authorized():
        return jsonify(error="unauthorized"), 401
    reports = []
    if DIAGNOSTICS_DIR.is_dir():
        for report_path in sorted(DIAGNOSTICS_DIR.glob("macos-startup-*.json"), reverse=True):
            reports.append({"name": report_path.name, "size": report_path.stat().st_size})
    return render_template_string(
        """<!doctype html><title>Astro-Nex diagnostics</title>
        <h1>Astro-Nex diagnostics</h1>
        <p>{{ reports|length }} report(s)</p>
        <ul>{% for report in reports %}<li><a href="/diagnostics/{{ report.name }}?key={{ key }}">{{ report.name }}</a> ({{ report.size }} bytes)</li>{% endfor %}</ul>""",
        reports=reports,
        key=request.args["key"],
    )


@app.get("/diagnostics/<report_name>")
def diagnostics_report(report_name: str) -> Response:
    if not _diagnostic_browser_authorized():
        return jsonify(error="unauthorized"), 401
    if Path(report_name).name != report_name or not report_name.startswith("macos-startup-") or not report_name.endswith(".json"):
        return jsonify(error="not_found"), 404
    report_path = DIAGNOSTICS_DIR / report_name
    if not report_path.is_file():
        return jsonify(error="not_found"), 404
    try:
        report = json.loads(report_path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError):
        return jsonify(error="invalid_report"), 500
    return render_template_string(
        """<!doctype html><title>{{ name }}</title>
        <p><a href="/diagnostics?key={{ key }}">All reports</a></p>
        <h1>{{ name }}</h1><h2>Client</h2><pre>{{ client|tojson(indent=2) }}</pre><h2>Log</h2><pre>{{ log }}</pre>""",
        name=report_name,
        key=request.args["key"],
...
if __name__ == "__main__":
    ...
    # El renderer se mantiene serializado por RENDER_LOCK, pero Flask puede
    # atender diagnósticos y health checks mientras una carta tarda en renderizar.
