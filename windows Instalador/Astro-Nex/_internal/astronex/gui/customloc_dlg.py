...
class CustomLocDlg(gtk.Dialog):
    def __init__(self,boss):
        ...
        self.loc_widget = CustomLocWidget(self)
        frame = gtk.Frame()
        frame.add(self.loc_widget)
    ...
    def dlg_response(self,dialog,rid):
        if rid == gtk.RESPONSE_OK:
            resp = self.loc_widget.pack_response()
    ...
    def on_modify_entry(self,but):
        ...
        self.loc_widget.gcombos[0].set_active(['E','W'].index(L))
        self.loc_widget.gentries[0].set_text(".".join([d.rjust(3,'0'),m,'00']))
        L = ''
        d,L,m = lat.partition('N')
        if L == '':
            d,L,m = lat.partition('S') 
        self.loc_widget.gcombos[1].set_active(['N','S'].index(L)) 
        self.loc_widget.gentries[1].set_text(".".join([d,m,'00']))
        _, count = table.split('_')
        if count.startswith('US'):
            self.loc_widget.check.set_active(True)
            reg = datab.get_usadistrict_from_code(count[2:],code)
        else:
            self.loc_widget.check.set_active(False)
            reg = datab.get_regionname_from_code(count.upper(),code)
        model = self.loc_widget.reg_combo.get_model()
        for r in model:
            if r[0] == reg:
                self.loc_widget.reg_combo.set_active_iter(r.iter)
                break
